...
export const LayerPanel = () => {
    ...
    return (_jsxs("div", { style: {
            ...
            overflow: 'hidden',
                    ...
                    letterSpacing: 0.5,
                }, children: "Layers" }), _jsx("div", { style: { flex: 1, overflowY: 'auto' }, children: layers.map((layer, index) => {
                    const isActive = layer.name === activeLayer;
                    return (_jsxs("div", { draggable: true, onDragStart: () => handleDragStart(index), onDragOver: (e) => handleDragOver(e, index), onDragEnd: handleDragEnd, onClick: () => setActiveLayer(layer.name), style: {
                            display: 'flex',
                            alignItems: 'center',
                            gap: 8,
                            padding: '4px 12px',
                            ...
                            if (!isActive) {
                                e.currentTarget.style.backgroundColor = 'transparent';
                        ...
                        }, children: [_jsx(Icon, { name: "drag", size: 12, color: theme.colors.textSecondary }), _jsx("div", { style: {
                                    width: 14,
                                    height: 14,
                                    borderRadius: 2,
                                ...
                                }, children: layer.name }), _jsx("button", { onClick: (e) => {
                                    e.stopPropagation();
                                    ...
                                    display: 'flex',
                                    alignItems: 'center',
                                }, children: _jsx(Icon, { name: layer.visible ? 'eye' : 'eye-off', size: 14, color: layer.visible ? theme.colors.text : theme.colors.textSecondary }) })] }, layer.id));
                }) })] }));
};
