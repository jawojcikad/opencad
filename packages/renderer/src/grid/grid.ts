 ...
 */
export function snapToGrid(point: Vector2D, gridSpacing: number): Vector2D {
  return new Vector2D(
...
export class GridRenderer {
  ...
  renderCanvas2D(ctx: CanvasRenderingContext2D, camera: Camera): void {
    ...
    for (let x = startX; x <= endX; x += minorSpacing) {
      if (isMultipleOf(x, majorSpacing)) continue; // drawn separately
    ...
    for (let y = startY; y <= endY; y += minorSpacing) {
      if (isMultipleOf(y, majorSpacing)) continue;
