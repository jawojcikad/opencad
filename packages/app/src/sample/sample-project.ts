...
function pad(
  ...
  w: number,
  h: number,
  shape: PadDef['shape'] = 'rect',
  type: PadDef['type'] = 'smd',
  drill?: number,
): PadDef {
...
function createBattery(ref: string, x: number, y: number): SchematicComponent {
  return {
    id: uid('comp'),
    symbolId: 'battery',
    reference: ref,
...
function createResistor(ref: string, value: string, x: number, y: number, rotation = 0): SchematicComponent {
  return {
    id: uid('comp'),
    symbolId: 'resistor',
    reference: ref,
...
function createCapacitor(ref: string, value: string, x: number, y: number): SchematicComponent {
  return {
    id: uid('comp'),
    symbolId: 'capacitor',
    reference: ref,
...
function create555(ref: string, x: number, y: number): SchematicComponent {
  return {
    id: uid('comp'),
    symbolId: 'ne555',
    reference: ref,
...
function createLED(ref: string, color: string, x: number, y: number): SchematicComponent {
  return {
    id: uid('comp'),
    symbolId: 'led',
    reference: ref,
...
function dip8Footprint(
  ...
  for (let i = 0; i < 4; i++) {
    pads.push(
      pad(`pad-${i + 1}`, padNets[i] ?? '', -rowSpacing / 2, -pitch * 1.5 + pitch * i, 1.6, 1.6, 'circle', 'through-hole', 0.8),
    );
  }
  for (let i = 0; i < 4; i++) {
    pads.push(
      pad(`pad-${8 - i}`, padNets[7 - i] ?? '', rowSpacing / 2, -pitch * 1.5 + pitch * i, 1.6, 1.6, 'circle', 'through-hole', 0.8),
    );
  }
...
export function createSampleProject(): OpenCADProjectFile {
  _idCounter = 0;
  ...
  const components: SchematicComponent[] = [battery, r1, r2, c1, ic1, r3, led];
  ...
  const pcbFootprints: FootprintDef[] = [
