...
function segmentToSegmentDist(a1, a2, b1, b2) {
    return Math.min(pointToSegmentDist(a1, b1, b2), pointToSegmentDist(a2, b1, b2), pointToSegmentDist(b1, a1, a2), pointToSegmentDist(b2, a1, a2));
}
function generateId() {
...
// DRC checks
// ---------------------------------------------------------------------------
function checkMinTrackWidth(tracks, rules) {
    const violations = [];
...
function checkMinDrill(vias, rules) {
    const violations = [];
    for (const v of vias) {
...
function checkTrackClearance(tracks, rules) {
    const violations = [];
        ...
        for (let j = i + 1; j < tracks.length; j++) {
            ...
            for (let sa = 0; sa < ta.points.length - 1; sa++) {
                for (let sb = 0; sb < tb.points.length - 1; sb++) {
                    const dist = segmentToSegmentDist(ta.points[sa], ta.points[sa + 1], tb.points[sb], tb.points[sb + 1]);
...
function checkPadClearance(footprints, rules) {
    const violations = [];
    ...
    for (let i = 0; i < allPads.length; i++) {
        for (let j = i + 1; j < allPads.length; j++) {
            const a = allPads[i];
            const b = allPads[j];
            if (a.pad.net === b.pad.net)
                continue;
            const dist = distance(a.absPos, b.absPos);
            const minSize = Math.min(Math.max(a.pad.size.x, a.pad.size.y), Math.max(b.pad.size.x, b.pad.size.y));
...
export {};
//# sourceMappingURL=drc-worker.js.map