...
export interface AutorouterConfig {
  gridResolution: number; // mm per grid cell (e.g. 0.5)
  maxIterations: number;
  preferredLayer: string;
}

// ---------------------------------------------------------------------------
// Grid-based Lee Router
// ---------------------------------------------------------------------------
...
function buildConnectionList(footprints: FootprintDef[]): NetConnection[] {
  // Group pads by net
  const netPads = new Map<string, Vec2[]>();
  for (const fp of footprints) {
    for (const pad of fp.pads) {
      if (!pad.net || pad.net === '') continue;
      const absPos: Vec2 = {
...
function worldToGrid(pos: Vec2, origin: Vec2, res: number): { r: number; c: number } {
  return {
...
function gridToWorld(r: number, c: number, origin: Vec2, res: number): Vec2 {
...
function leeRoute(
  ...
  while (queue.length > 0) {
    ...
    for (const d of dirs) {
      ...
      if (cell.visited || cell.blocked) continue;
  ...
  let cur: { r: number; c: number } | null = { r: endR, c: endC };
  while (cur) {
    pathCells.push(cur);
  ...
  const simplified: { r: number; c: number }[] = [pathCells[0]];
...
self.onmessage = (
  e: MessageEvent<{ document: PCBDocument; rules: DesignRules; config: AutorouterConfig }>,
) => {
  ...
  for (let i = 0; i < connections.length; i++) {
    ...
    if (grid[sg.r]?.[sg.c]) grid[sg.r][sg.c].blocked = false;
    if (grid[eg.r]?.[eg.c]) grid[eg.r][eg.c].blocked = false;
    ...
    if (grid[sg.r]?.[sg.c]) grid[sg.r][sg.c].blocked = startBlocked;
    if (grid[eg.r]?.[eg.c]) grid[eg.r][eg.c].blocked = endBlocked;
