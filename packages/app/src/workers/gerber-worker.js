...
function gerberHeader(layerName) {
    const lines = [
...
class ApertureTable {
    apertures = [];
    nextCode = 10; // D10 is the first user aperture
    getOrCreate(type, params) {
        // Look for existing
...
// Layer generators
// ---------------------------------------------------------------------------
function generateCopperLayer(layerName, footprints, tracks, vias) {
    ...
    for (const fp of footprints) {
        if (fp.layer !== layerName && layerName !== 'F.Cu' && layerName !== 'B.Cu')
            continue;
...
function generateSilkscreen(layerName, footprints) {
    const apt = new ApertureTable();
    const commands = [];
...
function generateBoardOutline(outline) {
    const apt = new ApertureTable();
...
function generateExcellonDrill(footprints, vias) {
    const lines = [
...
export {};
//# sourceMappingURL=gerber-worker.js.map