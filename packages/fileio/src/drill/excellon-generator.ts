...
export class ExcellonGenerator {
   ...
   */
  private collectHoles(document: PCBDocument): DrillHole[] {
    ...
    for (const fp of footprints) {
      ...
      for (const pad of pads) {
        ...
        if (drill === undefined || drill <= 0) continue;
        ...
        const plated: boolean = (pad as any).plated !== false;
   ...
   * tool number.
   */
  private groupByDiameter(holes: DrillHole[]): Map<number, DrillHole[]> {
    const map = new Map<number, DrillHole[]>();
   ...
   */
  private formatHeader(tools: Map<number, number>): string {
    const lines: string[] = [
      'M48',
  ...
  private buildFile(holes: DrillHole[], label?: string): string {
    if (holes.length === 0) {
    ...
    const toolMap = new Map<number, number>(); // diameter → toolNum
    const toolDiameters = new Map<number, number>(); // toolNum → diameter
    let toolIndex = 1;
    ...
    for (const diameter of sortedDiameters) {
      ...
      for (const hole of group) {
        lines.push(
          `X${this.formatCoord(hole.x)}Y${this.formatCoord(hole.y)}`,
        );
      }
    }
  ...
  private padTool(n: number): string {
