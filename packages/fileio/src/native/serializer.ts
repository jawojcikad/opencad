...
export class NativeSerializer {
  ...
  static serialize(project: OpenCADProjectFile): string {
    const output: OpenCADProjectFile = {
      ...project,
  ...
  static deserialize(json: string): OpenCADProjectFile {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse OpenCAD project JSON: ${(err as Error).message}`,
      );
    }

    if (!NativeSerializer.validateProject(data)) {
      throw new Error(
        'Invalid OpenCAD project file: missing or malformed required fields',
      );
    }
  ...
  static validateProject(data: unknown): data is OpenCADProjectFile {
    ...
    if (typeof obj['created'] !== 'string') return false;
    if (typeof obj['modified'] !== 'string') return false;

    // Schematic & PCB must be objects
    if (obj['schematic'] === null || typeof obj['schematic'] !== 'object')
      return false;
    if (obj['pcb'] === null || typeof obj['pcb'] !== 'object') return false;

    // Library section
    const lib = obj['library'];
    if (lib === null || typeof lib !== 'object') return false;
    const libObj = lib as Record<string, unknown>;
    if (!Array.isArray(libObj['symbols'])) return false;
    if (!Array.isArray(libObj['footprints'])) return false;
    if (!Array.isArray(libObj['components'])) return false;
    ...
    if (major > currentMajor) {
      ...
      console.warn(
        `Project version ${obj['version']} is newer than supported ${CURRENT_VERSION}`,
      );
    }
