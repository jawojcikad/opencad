...
import { WebGLRenderer, snapToGrid } from '@opencad/renderer';
import { InteractiveRouter } from '../routing/interactive-router';
import {
  PlaceFootprintCommand,
...
// Forward reference to avoid full circular dependency
  ...
  renderPreview(renderer: WebGLRenderer): void {
    if (this.isBoxSelecting) {
      ...
      renderer.drawRect(
        { x: minX, y: minY },
        { x: maxX, y: maxY },
        { r: 0.2, g: 0.5, b: 1.0, a: 0.15 },
        { r: 0.2, g: 0.5, b: 1.0, a: 0.6 },
        1,
      );
    }
  }
  ...
  private rotateSelection(): void {
    ...
    for (const id of this.editor.getSelection()) {
      ...
      if (fp) {
        ...
        const newRot = oldRot + 90; // 90 degrees
...
export class RouteTool implements PCBTool {
  public readonly name = 'route';
  public readonly cursor = 'crosshair';

  private editor: PCBEditor | null = null;
  private router: InteractiveRouter | null = null;
  private previewSegments: Array<{
    start: Vector2D;
    end: Vector2D;
    width: number;
    layer: Layer;
  }> = [];
  private currentWidth: number = 0.25;
  ...
  onMouseDown(worldPos: Vector2D, e: MouseEvent): void {
    if (!this.editor || !this.router) return;
  ...
  onMouseMove(worldPos: Vector2D, e: MouseEvent): void {
    if (!this.editor || !this.router || !this.router.getIsRouting()) return;
        ...
        if (this.router.getIsRouting()) {
          ...
          if (lastPt) {
            ...
            const via: Via = {
              ...
              layers: ['F.Cu' as Layer, 'B.Cu' as Layer],
            };
            const doc = this.editor.getDocument();
            ...
            const newLayer =
              currentLayer === ('F.Cu' as Layer) ? ('B.Cu' as Layer) : ('F.Cu' as Layer);
  ...
  private getLayerColor(layer: Layer): { r: number; g: number; b: number } {
...
export class PlaceFootprintTool implements PCBTool {
  public readonly name = 'place-footprint';
  public readonly cursor = 'copy';

  private editor: PCBEditor | null = null;
  private footprintTemplate: Footprint | null = null;
  private ghostPosition: Vector2D = { x: 0, y: 0 };
  ...
  onMouseDown(worldPos: Vector2D, e: MouseEvent): void {
    if (!this.editor || !this.footprintTemplate) return;
  ...
  renderPreview(renderer: WebGLRenderer): void {
    if (!this.footprintTemplate) return;

    // Draw ghost footprint at cursor position
    const fp = this.footprintTemplate;
    const cos = Math.cos((this.ghostRotation * Math.PI) / 180);
    const sin = Math.sin((this.ghostRotation * Math.PI) / 180);
...
export class DrawBoardOutlineTool implements PCBTool {
  public readonly name = 'board-outline';
  public readonly cursor = 'crosshair';

  private editor: PCBEditor | null = null;
  private points: Vector2D[] = [];
  private currentPoint: Vector2D = { x: 0, y: 0 };
  ...
  private finishOutline(): void {
    if (!this.editor || this.points.length < 3) return;
...
export class PlaceViaTool implements PCBTool {
  public readonly name = 'place-via';
  public readonly cursor = 'crosshair';

  private editor: PCBEditor | null = null;
  private previewPosition: Vector2D = { x: 0, y: 0 };
...
export class MeasureTool implements PCBTool {
  public readonly name = 'measure';
  public readonly cursor = 'crosshair';

  private editor: PCBEditor | null = null;
  private startPoint: Vector2D | null = null;
  private endPoint: Vector2D = { x: 0, y: 0 };
  private measurements: Array<{ start: Vector2D; end: Vector2D; distance: number }> =
    [];
