...
export class DRCChecker {
  constructor(private rules: DesignRules) {}
   ...
   * only nearby objects.
   */
  private checkClearances(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      for (let j = i + 1; j < objects.length; j++) {
        ...
        // Skip same net
        if (a.netId === b.netId && a.netId !== '') continue;
   ...
   */
  private checkMinTrackWidth(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];
   ...
   */
  private checkMinViaDimensions(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];
   ...
   */
  private checkMinHoleToHole(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];
    const minHoleToHole = this.rules.minHoleToHole ?? 0.5;

    // Collect all holes (vias + through-hole pads)
    const holes: Array<{ id: string; position: Vector2D; drill: number }> = [];
    ...
    for (const footprint of document.footprints) {
      ...
      for (const pad of footprint.pads) {
        if (pad.type !== 'through_hole' || !pad.drill) continue;
   ...
   */
  private checkUnroutedNets(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];

    // Build map of net -> pad positions
    const netPads = new Map<string, Array<{ id: string; position: Vector2D }>>();
    for (const footprint of document.footprints) {
      const cos = Math.cos(footprint.rotation);
      const sin = Math.sin(footprint.rotation);
    ...
    for (const [netId, pads] of netPads) {
      ...
      const getIdx = (pos: Vector2D): number => {
      ...
      for (const via of netVias) {
        ...
        for (const track of netTracks) {
          if (
            Math.abs(track.start.x - via.position.x) < SNAP_TOL &&
            Math.abs(track.start.y - via.position.y) < SNAP_TOL
          ) {
            unite(viaIdx, getIdx(track.start));
          }
          if (
            Math.abs(track.end.x - via.position.x) < SNAP_TOL &&
   ...
   */
  private checkSilkOverPad(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];

    // Collect all pad bounding boxes
    const padBoxes: Array<{ id: string; bbox: BBox; layer: Layer }> = [];
    for (const footprint of document.footprints) {
      const cos = Math.cos(footprint.rotation);
      const sin = Math.sin(footprint.rotation);
      ...
      for (const pad of footprint.pads) {
        ...
        const padLayer = pad.layer ?? ('F.Cu' as Layer);
        padBoxes.push({
          id: pad.id,
          bbox: {
            minX: worldPos.x - hw,
    ...
    for (const footprint of document.footprints) {
      if (!footprint.silkscreen) continue;

      const cos = Math.cos(footprint.rotation);
      const sin = Math.sin(footprint.rotation);

      for (const silk of footprint.silkscreen) {
        const silkWorld: Vector2D = {
        ...
        const matchLayer =
          silkLayer === ('F.SilkS' as Layer) ? ('F.Cu' as Layer) : ('B.Cu' as Layer);
        ...
        for (const pad of padBoxes) {
          ...
          if (pad.layer !== matchLayer && pad.layer !== ('*' as Layer)) continue;
   ...
   */
  private checkCourtyardOverlap(document: PCBDocument): DRCViolation[] {
    const violations: DRCViolation[] = [];

    // Compute courtyard bounding boxes for all footprints
    const courtyards: Array<{ id: string; bbox: BBox; center: Vector2D }> = [];
  ...
  private computeFootprintCourtyard(footprint: Footprint): BBox | null {
    ...
    if (!footprint.pads || footprint.pads.length === 0) return null;
  ...
  private getAllCopperLayers(document: PCBDocument): Layer[] {
    const layers = new Set<Layer>();
    layers.add('F.Cu' as Layer);
    layers.add('B.Cu' as Layer);
    for (const track of document.tracks) {
