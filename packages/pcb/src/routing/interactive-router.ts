...
export class InteractiveRouter {
  ...
  private routeStartPoint: Vector2D = { x: 0, y: 0 };
  private currentLayer: Layer = 'F.Cu' as Layer;
  private currentNetId: string = '';
  private currentWidth: number = 0.25; // mm default
  private routeSegments: TrackSegment[] = [];
  private committedPoints: Vector2D[] = [];
  private routingMode: '45deg' | '90deg' = '45deg';
  ...
  updateRoute(currentPoint: Vector2D): TrackSegment[] {
    ...
    const previewSegments: TrackSegment[] = [];
  ...
  commitRoute(): Track[] {
    ...
    const tracks: Track[] = this.routeSegments.map((seg, i) => ({
  ...
  getLastPoint(): Vector2D | null {
    if (this.committedPoints.length === 0) return null;
  ...
  validateSegment(start: Vector2D, end: Vector2D, width: number, layer: Layer): boolean {
    ...
    for (const track of this.document.tracks) {
      ...
      if (track.netId === this.currentNetId) continue; // Same net, no clearance needed
    ...
    for (const footprint of this.document.footprints) {
      ...
      for (const pad of footprint.pads) {
        if (pad.netId === this.currentNetId) continue;
    ...
    for (const via of this.document.vias) {
      if (via.netId === this.currentNetId) continue;
   ...
   */
  private calculateRoutePath(start: Vector2D, end: Vector2D, mode: '45deg' | '90deg'): Vector2D[] {
    ...
    if (mode === '90deg') {
      ...
      const midPoint: Vector2D = Math.abs(dx) > Math.abs(dy)
        ? { x: end.x, y: start.y }
        : { x: start.x, y: end.y };
...
function cross(a: Vector2D, b: Vector2D, c: Vector2D): number {
  return (b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x);
}

function onSegment(a: Vector2D, b: Vector2D, p: Vector2D): boolean {
  return (
