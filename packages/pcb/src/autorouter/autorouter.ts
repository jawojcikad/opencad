...
interface GridNode {
  x: number;
  y: number;
  layer: number; // index into copper layers
}
...
export class Autorouter {
  ...
  private obstacleGrid: Map<string, boolean> = new Map(); // "x,y,layer" -> blocked
  private gridMinX: number = 0;
  private gridMinY: number = 0;
  private gridMaxX: number = 100;
  private gridMaxY: number = 100;
  private copperLayers: Layer[] = ['F.Cu' as Layer, 'B.Cu' as Layer];
  ...
  private findPath(
    ...
    const nodeKey = (x: number, y: number, layer: number): string =>
    ...
    while (openSet.size() > 0 && iterations < maxIter) {
      ...
      for (const dir of directions) {
        ...
        if (nx < this.gridMinX || nx > this.gridMaxX) continue;
        if (ny < this.gridMinY || ny > this.gridMaxY) continue;
        if (this.isBlocked(nx, ny, current.layer)) continue;
      ...
      if (this.copperLayers.length > 1) {
        for (let layerIdx = 0; layerIdx < this.copperLayers.length; layerIdx++) {
          if (layerIdx === current.layer) continue;

          const nKey = nodeKey(current.x, current.y, layerIdx);
          if (closedSet.has(nKey)) continue;
          if (this.isBlocked(current.x, current.y, layerIdx)) continue;
  ...
  private heuristic(
    ...
    const manhattan =
      (Math.abs(x - ex) + Math.abs(y - ey)) * this.config.costFactors.length;
    const layerCost =
      layer !== endLayer && endLayer !== -1
   ...
   */
  private pathToTracks(path: AStarNode[], netId: string): Track[] {
    const tracks: Track[] = [];
    ...
    if (
      Math.abs(segStart.x - lastPath.x) > 1e-6 ||
        ...
        for (let gx = worldPos.x - hw; gx <= worldPos.x + hw; gx += res) {
          for (let gy = worldPos.y - hh; gy <= worldPos.y + hh; gy += res) {
            ...
            if (pad.type === 'through_hole') {
              ...
              const layerIdx = this.getLayerIndex(pad.layer ?? ('F.Cu' as Layer));
              this.obstacleGrid.set(
                `${snappedX.toFixed(4)},${snappedY.toFixed(4)},${layerIdx}`,
                true,
              );
            }
          }
  ...
  private addTrackToObstacleGrid(track: Track): void {
    const res = this.config.gridResolution;
    const clearance = this.rules.clearance ?? 0.2;
  ...
  private isBlocked(x: number, y: number, layer: number): boolean {
  ...
  private initializeBounds(): void {
    ...
    if (this.document.boardOutline && this.document.boardOutline.points.length > 0) {
      ...
      for (const fp of this.document.footprints) {
        if (fp.position.x < minX) minX = fp.position.x;
        if (fp.position.y < minY) minY = fp.position.y;
        if (fp.position.x > maxX) maxX = fp.position.x;
        if (fp.position.y > maxY) maxY = fp.position.y;
      }
      if (minX === Infinity) {
        minX = 0;
        minY = 0;
    ...
    layerSet.add('B.Cu');
    for (const track of this.document.tracks) {
      layerSet.add(track.layer as string);
    }
    this.copperLayers = Array.from(layerSet) as Layer[];
  }
  ...
  private collectNetPads(
    netId: string,
  ): Array<{ position: Vector2D; id: string; layer: Layer }> {
    const pads: Array<{ position: Vector2D; id: string; layer: Layer }> = [];
   ...
   */
  private collectUnroutedNets(): Array<{ netId: string; manhattanDist: number }> {
    const netPads = new Map<string, Vector2D[]>();
    ...
    const result: Array<{ netId: string; manhattanDist: number }> = [];
   ...
   */
  private computeMST(
    pads: Array<{ position: Vector2D; id: string; layer: Layer }>,
