...
export class ZoneFiller {
  ...
  fillZone(zone: CopperZone, document: PCBDocument): Vector2D[][] {
    ...
    const settings: ZoneFillSettings = zone.fillSettings ?? {
      clearance: 0.3,
      minWidth: 0.2,
  ...
  offsetPolygon(polygon: Vector2D[], offset: number): Vector2D[] {
    ...
    const offsetEdges: Array<{ p: Vector2D; d: Vector2D }> = [];
    for (let i = 0; i < n; i++) {
  ...
  subtractObstacles(zone: Vector2D[], obstacles: BBox[], clearance: number): Vector2D[][] {
    let currentPolygons: Vector2D[][] = [zone];
   ...
   */
  private computePolygonDifference(polygon: Vector2D[], rect: Vector2D[]): Vector2D[][] {
    ...
    intersections.sort((a, b) =>
      a.polyEdge !== b.polyEdge ? a.polyEdge - b.polyEdge : a.polyT - b.polyT,
    );
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (const ix of edgeIntersections) {
        ...
        if (!nextIsInside) {
          ...
          const nextIx =
            exitIdx + 1 < intersections.length
              ? intersections[exitIdx + 1]
              : intersections[0];

          // Walk along rect boundary from entry to exit
          const boundaryPoints = this.walkRectBoundary(
            rect,
            ix.point,
            nextIx.point,
            ix.rectEdge,
   ...
   */
  private createPolygonWithHole(outer: Vector2D[], hole: Vector2D[]): Vector2D[] {
  ...
  private collectObstacles(
    ...
    for (const otherZone of document.copperZones) {
      ...
      if (otherZone.layer !== layer) continue;
      if (otherZone.netId === netId) continue;

      const otherSettings: ZoneFillSettings = otherZone.fillSettings ?? {
        clearance: 0.3,
        minWidth: 0.2,
        thermalReliefGap: 0.5,
        thermalReliefWidth: 0.25,
...
function polygonArea(polygon: Vector2D[]): number {
  let area = 0;
  const n = polygon.length;
  for (let i = 0; i < n; i++) {
    const j = (i + 1) % n;
...
function computeBBox(polygon: Vector2D[]): BBox {
  let minX = Infinity,
    minY = Infinity,
    maxX = -Infinity,
    maxY = -Infinity;
...
function pointInRect(p: Vector2D, rect: BBox): boolean {
  return p.x >= rect.minX && p.x <= rect.maxX && p.y >= rect.minY && p.y <= rect.maxY;
}

function pointInPolygon(p: Vector2D, polygon: Vector2D[]): boolean {
  let inside = false;
  const n = polygon.length;
 ...
 */
function sutherlandHodgmanClip(subject: Vector2D[], clip: Vector2D[]): Vector2D[] {
  let output: Vector2D[] = [...subject];
...
function isInsideEdge(p: Vector2D, edgeStart: Vector2D, edgeEnd: Vector2D): boolean {
  return (
...
function polygonScanlineIntersections(polygon: Vector2D[], y: number): number[] {
