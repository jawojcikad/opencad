...
interface RTreeLeaf<T extends SpatialItem> {
  kind: 'leaf';
  bbox: BBox;
  children: T[];
}

interface RTreeBranch<T extends SpatialItem> {
  kind: 'branch';
  bbox: BBox;
  children: RTreeNode<T>[];
}

type RTreeNode<T extends SpatialItem> = RTreeLeaf<T> | RTreeBranch<T>;
...
function combinedBBox(boxes: { bbox: BBox }[]): BBox {
...
function bboxEnlargement(base: BBox, addition: BBox): number {
 ...
 */
export class RTree<T extends SpatialItem> {
  ...
  private splitLeaf(node: RTreeLeaf<T>): [RTreeLeaf<T>, RTreeLeaf<T>] {
    const [seedA, seedB] = this.pickSeeds(node.children);
    const a: T[] = [node.children[seedA]];
    const b: T[] = [node.children[seedB]];
  ...
  private splitBranch(
    node: RTreeBranch<T>,
  ): [RTreeBranch<T>, RTreeBranch<T>] {
    const [seedA, seedB] = this.pickSeeds(node.children);
    const a: RTreeNode<T>[] = [node.children[seedA]];
    const b: RTreeNode<T>[] = [node.children[seedB]];
    let bboxA = node.children[seedA].bbox;
    let bboxB = node.children[seedB].bbox;
  ...
  private pickSeeds(items: { bbox: BBox }[]): [number, number] {
    ...
    for (let i = 0; i < items.length; i++) {
      for (let j = i + 1; j < items.length; j++) {
        ...
        const waste =
          bboxArea(combined) - bboxArea(items[i].bbox) - bboxArea(items[j].bbox);
        if (waste > worstWaste) {
          worstWaste = waste;
          seedA = i;
          seedB = j;
  ...
  private removeFromNode(node: RTreeNode<T>, id: string): boolean {
    if (node.kind === 'leaf') {
    ...
    for (const child of node.children) {
      if (this.removeFromNode(child, id)) {
        ...
        if (
          (child.kind === 'leaf' && child.children.length < MIN_CHILDREN && child.children.length > 0) ||
    ...
    node.children = node.children.filter((c) => {
      if (c.kind === 'leaf') return c.children.length > 0;
      if (c.kind === 'branch') return c.children.length > 0;
      return true;
    });
  ...
  private collectItems(node: RTreeNode<T>): T[] {
    if (node.kind === 'leaf') return [...node.children];
    const items: T[] = [];
    for (const child of node.children) {
  ...
  private searchNode(node: RTreeNode<T>, bbox: BBox, results: T[]): void {
    if (!node.bbox.intersects(bbox)) return;
