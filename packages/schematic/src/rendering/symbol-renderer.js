...
export class SymbolRenderer {
    static PIN_LENGTH = 20;
    static PIN_RADIUS = 3;
    static BODY_COLOR = '#333333';
    static BODY_FILL = '#fffff0';
    static PIN_COLOR = '#cc0000';
    static SELECTED_COLOR = '#0088ff';
    static LABEL_FONT = '11px monospace';
    static REF_FONT = 'bold 12px monospace';
    render(ctx, symbol, position, rotation, selected, mirrored = false) {
    ...
    // ── Body ───────────────────────────────────────────────────────────────────
    drawBody(ctx, symbol, _position, _rotation, selected) {
        const g = ctx.getContext();
        const strokeColor = selected
            ? SymbolRenderer.SELECTED_COLOR
            : SymbolRenderer.BODY_COLOR;
        const legacyGraphics = symbol.graphics;
            ...
            for (const graphic of legacyGraphics) {
                ...
                switch (graphic.type) {
                    ...
                    case 'ellipse': {
                        g.fillStyle = SymbolRenderer.BODY_FILL;
                        g.beginPath();
                        g.ellipse(graphic.cx, graphic.cy, graphic.rx, graphic.ry, 0, 0, Math.PI * 2);
                        g.fill();
                        g.stroke();
                        break;
                    }
                    ...
                    case 'arc': {
                        g.beginPath();
                        g.arc(graphic.cx, graphic.cy, graphic.radius, graphic.startAngle ?? 0, graphic.endAngle ?? Math.PI * 2);
                        g.stroke();
                        break;
                    }
                    case 'text': {
        ...
        else if ((symbol.lines && symbol.lines.length > 0) ||
    ...
    // ── Pins ───────────────────────────────────────────────────────────────────
    drawPins(ctx, pins, _position, _rotation, selected) {
        const g = ctx.getContext();
        for (const pin of pins) {
            ...
            let endX = px;
            let endY = py;
            const orientation = this.mapPinOrientation(pin.orientation);
            switch (orientation) {
                case 'left':
                    endX = px - pinLen;
                    break;
    ...
    drawPinLabels(ctx, pin, position, selected) {
        const g = ctx.getContext();
        const orientation = this.mapPinOrientation(pin.orientation);
        const nameOffset = 5;
        ...
        if (pin.number) {
            ...
            switch (orientation) {
                case 'left':
                    g.textAlign = 'right';
                    g.fillText(pin.number, position.x - pinLen + 2, position.y - 8);
                    break;
                case 'right':
                    g.textAlign = 'left';
                    g.fillText(pin.number, position.x + pinLen - 2, position.y - 8);
                ...
                case 'up':
                    g.textAlign = 'left';
                    g.fillText(pin.number, position.x + 4, position.y - pinLen + 2);
                    break;
                case 'down':
                    g.textAlign = 'left';
                    g.fillText(pin.number, position.x + 4, position.y + pinLen - 2);
