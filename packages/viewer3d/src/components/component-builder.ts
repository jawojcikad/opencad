...
export class ComponentBuilder {
  ...
  buildSOIC(pinCount: number, bodyWidth: number, bodyLength: number): THREE.Group {
    ...
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(bodyWidth, bodyLength, bodyHeight),
      this.componentBodyMaterial(0x1a1a1a),
    );
  ...
  buildQFP(pinCount: number, bodySize: number, pitch: number): THREE.Group {
    ...
    const body = new THREE.Mesh(
      new THREE.BoxGeometry(bodySize, bodySize, bodyHeight),
      this.componentBodyMaterial(0x1a1a1a),
    );
    body.position.z = bodyHeight / 2;
    group.add(body);
    ...
    for (let side = 0; side < 4; side++) {
      for (let i = 0; i < pinsPerSide; i++) {
        ...
        const pinGeo = new THREE.BoxGeometry(
          side % 2 === 0 ? pinWidth : pinLength,
          side % 2 === 0 ? pinLength : pinWidth,
          pinThickness,
        );
        const pin = new THREE.Mesh(pinGeo, this.pinMaterial());
    ...
    for (let side = 0; side < 2; side++) {
      ...
      for (let i = 0; i < pinsPerSide; i++) {
        ...
        const pin = new THREE.Mesh(
          new THREE.BoxGeometry(PIN_RADIUS * 2, PIN_RADIUS * 2, PIN_HEIGHT + bodyHeight),
          this.pinMaterial(),
        );
  ...
  autoGenerateComponent(footprint: Footprint): THREE.Group {
    ...
    if (/\bSOIC|SOP\b/.test(combined)) {
      ...
      const comp = this.buildSOIC(
        pinCount >= 4 ? pinCount : 8,
        3.9,
        pinCount * 0.635 + 1,
      );
  ...
  private componentBodyMaterial(color: number = 0x2a2a2a): THREE.MeshStandardMaterial {
